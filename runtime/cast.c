...
float rigg_str_to_f32(const char *s) {
  char *end = NULL;
  errno = 0;
  float v = strtof(s, &end);

  if (end == s || *end != '\0' || errno == ERANGE)
    rigg_runtime_error("invalid float string");

  return v;
}

double rigg_str_to_f64(const char *s) {
  char *end = NULL;
  errno = 0;
...
char *rigg_f32_to_str(float n) {
  char buf[64];
  snprintf(buf, sizeof(buf), "%.*g", FLT_DECIMAL_DIG, n);
  return rigg_strdup(buf);
}

char *rigg_f64_to_str(double n) {
  char buf[128];
  snprintf(buf, sizeof(buf), "%.*g", DBL_DECIMAL_DIG, n);
  return rigg_strdup(buf);
}

char *rigg_bool_to_str(int b) { return rigg_strdup(b ? "true" : "false"); }
...
int rigg_str_eq(const char *a, const char *b) { return strcmp(a, b) == 0; }

int rigg_str_ne(const char *a, const char *b) { return strcmp(a, b) != 0; }
